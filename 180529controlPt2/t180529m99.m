...
y5=imread('20180529_600_2.jpg');
% ie=1684+2196;je=150+3367;  % 影像左上角點(灰階30)  20180409_01.jpg
% ie=4003.6;je=162.6;  % 影像左下角點(灰階100)  20180409_02.jpg
 ie=56;je=135;  % 影像左上角點(灰階30)  20180529_600_2.jpg
% ie=72;je=872;  % 影像左上角點(灰階30)  20180529_600_1.jpg
image(y5);
%取座標 % [a,b] = ginput;
%   
ii=10; jj=4;         % <========== 選取位置
%
figure(1);imshow(y5(ie+1+(ii-1)*660:ie+ii*660,je+1+(jj-1)*660:je+jj*660,:));
...
newuv=[];
for nx=2/3 : 3/3 : 33+3/3
    for ny=2/3 : 3/3 : 33+3/3
        uv=A*[1; nx; ny];
...
j=round(newuv(1,1)); 
i=round(newuv(1,2));
figure(10),imshow(y5b(i-6:i+6,j-6:j+6))
...
t=imread('m180111_3.png');           % original QR data
t1=(t==0);
t2=imresize(t1,[37,37]);
t3=t2(3:35,3:35);
% imshow(t3);

recognition33 =sum(sum(data==t3))/33/33
figure(7),imshow(data==t3);title('error map');
...
% 33x33 error rate
 ...
 % x1=x(1:1:33,1:1:33);
 x1=x;
recognition33 =sum(sum(x1))/33/33
qr33_matrix_180523
