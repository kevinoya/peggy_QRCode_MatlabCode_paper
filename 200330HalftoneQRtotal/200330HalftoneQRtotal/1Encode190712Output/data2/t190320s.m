...
QRcodeA='post.png';  % <====    1st QR code filename (41x41)
QRcodeB='hmlvideo.png';     %  <====   2nd QR code filename (33x33)
key=9;
...
h=imread(QRcodeB);     %  <====  input hidden 2nd QR  (33x33)
h1=(h~=0);  % 164x164 binary   [ (4+33+4)*4  X  (4+33+4)*4 ]
h47=imresize(h1,[41 41],'nearest');  % 49x49
hh0=h47(5:37,5:37);   % 33x33 binary
hh=myQR33temp41(hh0);   % 41x41
%hh=imread('tempQR41.tif');  %  <====  input hidden 2nd QR
...
% k22=myQR41and41(k3a,y41,hh,9);   % <===== 主要雙重加密處理步驟
k22=myQR41and41mask(k3a,y41,hh,9);  %   <===== 有mask

imwrite(k22,'QR41s165.tif','Resolution',600);  % output binary 165x165
...
%
imwrite(k5,'QR41s660.tif','Resolution',600);  % output color 660x660x3
figure(2),imshow(k5);
...
% extract QR33
y9=myDecodeKey(k22(s+1:s+123,t+1:t+123),key);   % 41x41
x9=myQR41temp33(y9);
figure(4),imshow(x9);title('recovered QR33');
